...
class Config:
    def __init__(self):
        ...
        self.ENABLE_COOKIES_FALLBACK: bool = getenv("ENABLE_COOKIES_FALLBACK", "True").strip().lower() in ("true", "1", "yes")
        _cookie_raw = getenv("COOKIE_URL", "").strip()
        self.COOKIE_URLS: list = [u.strip() for u in _cookie_raw.split(",") if u.strip()] if _cookie_raw else []
