...
@app.on_message(cmd(["logger", "مسجل"]) & app.sudoers)
@lang.language()
async def _logger(_, m: types.Message):
    if len(m.command) < 2:
        return await m.reply_text(m.lang["logger_usage"].format(m.command[0]))
    if m.command[1] not in ("on", "off", "تفعيل", "تعطيل"):
        return await m.reply_text(m.lang["logger_usage"].format(m.command[0]))

    if m.command[1] in ("on", "تفعيل"):
...
@app.on_message(cmd(["restart", "اعد", "ريستارت"]) & app.sudoers)
@lang.language()
